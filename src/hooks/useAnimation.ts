  ...
  useEffect(() => {
    ...
    function tick(timestamp: number) {
      ...
      if (pace && elapsed > nextPaceChangeRef.current) {
        // Weighted random: 'both' appears more often for natural feel
      ...
      // Lerp toward targets so pace mode transitions don't snap
...
// Deterministic pseudo-random based on time seed (avoids Math.random in rAF)
